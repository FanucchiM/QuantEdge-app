...
export type SortColumn = 'companyName' | 'signalType' | 'market' | 'trend' | 'price' | 'priceChangePercent24h' | 'volumeRelative' | 'sector' | null;

export interface SortConfig {
  column: 'companyName' | 'signalType' | 'market' | 'trend' | 'price' | 'priceChangePercent24h' | 'volumeRelative' | 'sector';
  direction: 'asc' | 'desc';
}
...
@Component({
  ...
  template: `
    <div class="dashboard">
      ...
      <header class="header">
        <div class="logo">
          <img src="favicon_dark_512x512.png" class="logo-icon" alt="QuantEdge">
            ...
            <div class="header-row">
              ...
              <div class="header-controls">
                ...
                <div class="sort-menu" *ngIf="sortMenuOpen">
                  ...
                  <button 
                    class="sort-option" 
                    [class.active]="isColumnSorted('trend')"
                    (click)="sortBy('trend')">
                    Trend
                    <span class="sort-icon" *ngIf="isColumnSorted('trend')">
                      {{ isColumnSorted('trend') === 'asc' ? '↑' : '↓' }}
                    </span>
                  </button>
                  <button 
                    class="sort-option" 
                    [class.active]="isColumnSorted('price')"
                    (click)="sortBy('price')">
                    Price
                    <span class="sort-icon" *ngIf="isColumnSorted('price')">
                      {{ isColumnSorted('price') === 'asc' ? '↑' : '↓' }}
                    </span>
                  </button>
                  <button 
                    class="sort-option" 
                    [class.active]="isColumnSorted('priceChangePercent24h')"
                    (click)="sortBy('priceChangePercent24h')">
                    24h Change
                    <span class="sort-icon" *ngIf="isColumnSorted('priceChangePercent24h')">
                      {{ isColumnSorted('priceChangePercent24h') === 'asc' ? '↑' : '↓' }}
                    </span>
                  </button>
                  <button 
                    class="sort-option" 
                    [class.active]="isColumnSorted('volumeRelative')"
                    (click)="sortBy('volumeRelative')">
                    Volume
                    <span class="sort-icon" *ngIf="isColumnSorted('volumeRelative')">
                      {{ isColumnSorted('volumeRelative') === 'asc' ? '↑' : '↓' }}
                    </span>
                  </button>
                  <button 
                    class="sort-option" 
                    [class.active]="isColumnSorted('sector')"
                    (click)="sortBy('sector')">
                    Sector
                    <span class="sort-icon" *ngIf="isColumnSorted('sector')">
                      {{ isColumnSorted('sector') === 'asc' ? '↑' : '↓' }}
                    </span>
                  </button>
            ...
            <table class="signals-table">
              <thead>
                <tr>
                  ...
                  <th class="sortable" (click)="sortBy('trend', $event)">
                    <span class="th-text">Trend</span>
                    <span class="th-sort-icon" *ngIf="isColumnSorted('trend')">
                      {{ isColumnSorted('trend') === 'asc' ? '↑' : '↓' }}
                    </span>
                  </th>
                  <th class="sortable" (click)="sortBy('price', $event)">
                    <span class="th-text">Price</span>
                    <span class="th-sort-icon" *ngIf="isColumnSorted('price')">
                      {{ isColumnSorted('price') === 'asc' ? '↑' : '↓' }}
                    </span>
                  </th>
                  <th class="sortable" (click)="sortBy('priceChangePercent24h', $event)">
                    <span class="th-text">24h Change %</span>
                    <span class="th-sort-icon" *ngIf="isColumnSorted('priceChangePercent24h')">
                      {{ isColumnSorted('priceChangePercent24h') === 'asc' ? '↑' : '↓' }}
                    </span>
                  </th>
                  <th class="sortable" (click)="sortBy('volumeRelative', $event)">
                    <span class="th-text">VOL</span>
                    <span class="th-sort-icon" *ngIf="isColumnSorted('volumeRelative')">
                      {{ isColumnSorted('volumeRelative') === 'asc' ? '↑' : '↓' }}
                    </span>
                  </th>
                  <th class="sortable" (click)="sortBy('sector', $event)" style="width: 220px;">
                    <span class="th-text">Sector</span>
                    <span class="th-sort-icon" *ngIf="isColumnSorted('sector')">
                      {{ isColumnSorted('sector') === 'asc' ? '↑' : '↓' }}
                    </span>
                  </th>
                </tr>
...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  get sortedSignals(): Signal[] {
    ...
    return [...result].sort((a, b) => {
      for (const config of configs) {
        ...
        } else if (config.column === 'trend') {
          const trendOrder: Record<string, number> = { 'bullish': 1, 'neutral': 2, 'bearish': 3 };
          comparison = (trendOrder[a.trend] || 2) - (trendOrder[b.trend] || 2);
        } else if (config.column === 'price') {
          comparison = (a.price || 0) - (b.price || 0);
        } else if (config.column === 'priceChangePercent24h') {
          comparison = (a.priceChangePercent24h || 0) - (b.priceChangePercent24h || 0);
        } else if (config.column === 'volumeRelative') {
          comparison = (a.volumeRelative || 0) - (b.volumeRelative || 0);
        } else if (config.column === 'sector') {
          comparison = (a.sector || '').toLowerCase().localeCompare((b.sector || '').toLowerCase());
        }
  ...
  sortBy(column: 'companyName' | 'signalType' | 'market' | 'trend' | 'price' | 'priceChangePercent24h' | 'volumeRelative' | 'sector', event?: MouseEvent): void {
  ...
  getColumnLabel(column: string): string {
    const labels: Record<string, string> = {
      ...
      'trend': 'Trend',
      'price': 'Price',
      'priceChangePercent24h': '24h Change',
      'volumeRelative': 'Volume',
      'sector': 'Sector',
    };
...
return '⏳ HOLD - No clear signal. Wait for more confirmation.';
  ...
  getReasonCategory(reason: string): { color: string; label: string; type: string } {
    ...
    return { color: '#94a3b8', label: 'Other', type: 'other' };
  }
